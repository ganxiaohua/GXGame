//------------------------------------------------------------------------------
// <auto-generated>
// </auto-generated>
//------------------------------------------------------------------------------
using GameFrame;
using UnityEngine;
public static partial class AutoHP
{
    
    public static void AddHP(this ECSEntity ecsEntity)
    {
        ecsEntity.AddComponent<GXGame.HP>();
    }
    
    public static void AddHP(this ECSEntity ecsEntity,System.Int32 param)
    {
        var p  =  (GXGame.HP)ecsEntity.AddComponent<GXGame.HP>();
        p.Value = param;
    }
          
    public static GXGame.HP GetHP(this ECSEntity ecsEntity)
    {
        return (GXGame.HP)ecsEntity.GetComponent(ComponentsID<GXGame.HP>.TID);
    }
     
    public static ECSEntity SetHP(this ECSEntity ecsEntity,System.Int32 param)
    {
        var p = (GXGame.HP)ecsEntity.GetComponent(ComponentsID<GXGame.HP>.TID);
        p.Value = param;
        ((World)ecsEntity.Parent).Reactive(ComponentsID<GXGame.HP>.TID, ecsEntity);
        
        return ecsEntity;
    }
    
    public static ECSEntity AddOrSetHP(this ECSEntity ecsEntity,System.Int32 param)
    {
        var p = (GXGame.HP)ecsEntity.GetComponent(ComponentsID<GXGame.HP>.TID);
        if(p==null)
        {
           p = (GXGame.HP)(ecsEntity.AddComponent<GXGame.HP>());
        }
        p.Value = param;
        ((World)ecsEntity.Parent).Reactive(ComponentsID<GXGame.HP>.TID, ecsEntity);
        
        return ecsEntity;
    } 
         
}